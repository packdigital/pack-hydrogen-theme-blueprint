...
import type {BannerContentProps} from '~/sections/Banner/Banner.types';
...
export function BannerContent({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`rounded-lg p-4 ${headingWithBreaks || subheading ? 'bg-[rgba(29,61,148,0.7)]' : ''}`}
        >
          {headingWithBreaks && (
            <>
              {aboveTheFold ? (
                ...
                <h2 className="text-h1 text-shadow">{headingWithBreaks}</h2>
              )}
            </>
          )}

          {subheading && <p className="text-xl text-shadow">{subheading}</p>}
        </div>
