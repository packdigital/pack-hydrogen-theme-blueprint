...
export const formatGroupingWithOptions = ({
  ...
  try {
    ...
    const hasParentGroupWithProducts =
      validParentGroupProductHandles.length > 0;
    ...
    > = {};
    if (hasSubgroupsWithProducts) {
      subGroupOptionsMapsByIndex = Object.entries(
        subGroupProductsByIndex,
      ).reduce((acc, [sgIndex, sgProducts]) => {
    ...
    console.error('formatGroupingWithOptions:error:', error);
    return null;
  }
};
